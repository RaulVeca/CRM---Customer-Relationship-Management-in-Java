...
/**
 * Single email sign-in. There is one "Login" card: the entered email is resolved
 * by the backend against both the contacts and the admins table, and the role it
 * comes back with decides the destination — USER opens the client site, ADMIN
 * opens the admin portal. The roles never cross over: a client email can only
 * ever yield a USER session and an admin email an ADMIN session.
 */
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const session = await api.post<AuthSession>("/api/auth/login", { email: value });
      setSession(session);
